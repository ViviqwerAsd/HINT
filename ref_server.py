...
class RefServer:
    ...
    def auto_bsz_infer(self, model, input_ids, pred_func, small_bsz=0):
        ...
        try:
            rets = [
                pred_func(model, input_ids[i : i + sbsz])
            ...
            if small_bsz == 0:
                self.oom_count += 1
                if self.oom_count > 3:
                    self.small_bsz, self.oom_count = max(1, sbsz // 2), 0
            return ret
    ...
    def start(self):
        ...
        while True:
            ...
            if "end" not in d:
                if self.model is not None:
                    with torch.inference_mode():
                        logps = self.auto_bsz_infer(
                            self.model, d["inputs"].to(device), get_per_token_logps
                        )
            ...
            if random.random() < 0.1:
                print(
                    f"raw_queue: {self .raw_queue .qsize ()}, result_queue: {self .result_queue .qsize ()}"
                )


def build_parser():
    parser = argparse.ArgumentParser(
        description="Reference log-probability server for HINT."
    )
