...
def apply_no_vllm_patch(lsrl_instance):
    ...
    def _generation_loop(self):
        ...
        def gen_rewards(samples, items):
            ...
            for i, ans in enumerate(samples["answers"]):
                ...
                for reward_fn in self.reward_fns:
                    reward[reward_fn.__name__] = reward_fn(
        ...
        class RolloutProcessorBase:
            ...
            def run(self, items):
                ...
                group_avg_rewards = []
                for prompt, ganswers, grewards in zip(
                    samples["prompts"], groups["answers"], groups["rewards"]
                ):
        ...
        for it, item in enumerate(Q_data):
            ...
            print(
                f"[GEN {gen_rank }]  time: {time .time ()-tic :.2f}s    ",
                f'avg_rewards: {",".join (group_avg_rewards )}',
            )
    ...
    lsrl_instance.generation_mode = _generation_mode.__get__(
        lsrl_instance, type(lsrl_instance)
    )
    lsrl_instance.generation_loop = _generation_loop.__get__(
        lsrl_instance, type(lsrl_instance)
    )
