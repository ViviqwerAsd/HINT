...
class LSRL:
    ...
    def set_hint_prompt_fn(self, user_fn): self._hint_prompt_fn = user_fn
    def set_direct_hint_prompt_fn(self, user_fn): self._direct_hint_prompt_fn = user_fn
    def set_medium_hint_prompt_fn(self, user_fn): self._medium_hint_prompt_fn = user_fn
    def set_abstract_hint_prompt_fn(self, user_fn): self._abstract_hint_prompt_fn = user_fn
    ...
    def hint_prompt_fn(self, item): return self._hint_prompt_fn(self, item)
    def direct_hint_prompt_fn(self, item): return self._direct_hint_prompt_fn(self, item)
    def medium_hint_prompt_fn(self, item): return self._medium_hint_prompt_fn(self, item)
    def abstract_hint_prompt_fn(self, item): return self._abstract_hint_prompt_fn(self, item)
                ...
                def make_groups(items, rn):
                    ...
                    for group_idx, rewards_group in enumerate(groups['rewards']):
                        if all(r['total'] == 0 for r in rewards_group):
                            print(f"[GEN {gen_rank}] {group_idx} group's answers are all wrong, need to give hint")
                    ...
                    if len(reroll_items) > 0:
                        ...
                        for j, gidx in enumerate(reroll_indices):
                            ...
                            samples['answers'][start_idx:end_idx] = new_groups_answers[j]
                            samples['rewards'][start_idx:end_idx] = new_groups_rewards[j]

                    print(f"[GEN {gen_rank}] 每个group的total sum:")
                    for i, rewards_group in enumerate(groups['rewards']):
