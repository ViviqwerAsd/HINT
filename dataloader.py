...
def convert_to_batches(
    data, tokenizer, max_len=4096, mode="packing", batch_size=4, test=False
):
    def pad_and_tensorize(input_ids, labels, item=None, pad_length=0):
        if max(labels) == -100:
            print(
                f"Found labels all -100, skipping item ...  item length: {len (item ['input_ids'])}"
            )
            return None
        if pad_length > 0:
    ...
    if mode == "padding":
        for i in range(0, len(data), batch_size):
            ...
            for item in batch:
                ...
                clever_append(
                    padded_batch, pad_and_tensorize(input_ids, labels, item, blen)
                )
            if padded_batch:
                batches.append(
                    {
                        "input_ids": torch.cat(
                            [item["input_ids"] for item in padded_batch]
                        ),
                        "labels": torch.cat([item["labels"] for item in padded_batch]),
                    }
                )
            if test and len(batches) >= 3:
                break
    elif mode == "packing":
        ...
        for item in data:
            ...
            if len(packed_input_ids) + len(input_ids) > max_len:
                clever_append(
                    batches,
                    pad_and_tensorize(packed_input_ids, packed_labels, last_item),
                )
                packed_input_ids, packed_labels = [], []
        ...
        if packed_input_ids:
            clever_append(
                batches, pad_and_tensorize(packed_input_ids, packed_labels, last_item)
            )
    return batches
...
class SFTDataHandler:
    def __init__(
        ...
        if custom_convert_fn:
            ...
            if not isinstance(firstitem, dict) or len(
                firstitem.get("input_ids", "0")
            ) != len(firstitem.get("labels", "01")):
                print(firstitem)
                raise ValueError(
                    "custom_convert_fn should return {'input_ids': [...], 'labels': [...]} for each item"
                )
            ...
            firstitem = uadata[0]
            if (
                not isinstance(firstitem, dict)
                or not isinstance(firstitem.get("input"), str)
                or not isinstance(firstitem.get("output"), str)
            ):
                print(firstitem)
                raise ValueError(
                    "custom_ua_format_fn should return {'input': str, 'output': str} for each item"
                )
            self.data = lmap(lambda x: convert_to_token_ids(x, tokenizer), uadata)
        else:
            iodata = lmap(convert_to_io_format, data)
            uadata = lmap(
                lambda x: convert_to_ua_format(x, system_prompt=system_prompt), iodata
            )
            self.data = lmap(lambda x: convert_to_token_ids(x, tokenizer), uadata)

        if case_study:
            batches = convert_to_batches(
                self.data,
                self.tokenizer,
                self.max_len,
                self.mode,
    ...
    def display_case_study(self, batch):
        ...
        input_texts = lmap(
            lambda x: self.tokenizer.decode(x, skip_special_tokens=False), input_ids
        )
        dot = self.tokenizer.encode(".")[0]
        output_texts = lmap(
            lambda x: self.tokenizer.decode(
                [z if z > 0 else dot for z in x], skip_special_tokens=False
            ),
            labels,
        )
    ...
    def get_dataloader(self, shuffle=True):
        ...
        self.remake_num += 1
        batches = convert_to_batches(
            self.data, self.tokenizer, self.max_len, self.mode, self.batch_size
        )
...
if __name__ == "__main__":
    ...
    def my_convert_fn(item, tokenizer):
        ...
        return {"input_ids": inp, "labels": out}
