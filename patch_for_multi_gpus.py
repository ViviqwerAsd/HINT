...
def decoding_layer_forward(
    ...
    **kwargs,
    ...
    if cache_position is not None:
        cache_position = cache_position.to(obj_device)
...
def chunked_lm_head_forward(self, input_ids, labels, use_cache=False, **kwargs):
    ...
    chunk_size = (
        self._lm_head_chunk_size if hasattr(self, "_lm_head_chunk_size") else 1024
    )
...
def patch_qwen2_for_multi_gpus(model, devices, patch_lm_head=False, chunk_size=1024):
    ...
    print(
        f"Make sure that the input should in cuda:{devices [0 ]} and the output in cuda:{devices [-1 ]}..."
    )
