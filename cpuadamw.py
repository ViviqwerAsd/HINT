    ...
    def __init__(
        ...
        **kwargs,
        ...
        if grad_offload is None:
            ...
            print(
                "If you want to train long long sequences, consider manually set grad_offload=True."
            )
        else:
        ...
        self.cpu_optimizer = AdamW(
            cpu_params, lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, **kwargs
        )
        ...
        if self.grad_offload or is_update_step:
            ...
            for cpu_p, original_p in self.original_device_map.items():
                if original_p.grad is not None:
                    scaled_grad_future = (original_p.grad / self.accum_steps).to(
            ...
            for cpu_p, scaled_grad in async_grad_transfers:
                cpu_p.grad = (
                    scaled_grad if cpu_p.grad is None else cpu_p.grad + scaled_grad
                )
        if is_update_step:
            ...
            for p in self.gpu_params:
                if p.grad is not None:
                    torch.distributed.all_reduce(
                        p.grad.data, op=torch.distributed.ReduceOp.AVG
                    )
            if self.rank == 0:
                async_grad_transfers = []
                for cpu_p, original_p in self.original_device_map.items():
                ...
                for original_p in self.gpu_params:
                    original_p.grad = None
